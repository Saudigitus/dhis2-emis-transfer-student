...
function RequestTransfer(props: RequestTranferProps): React.ReactElement {
  ...
  const modalActions = [
    ...
    { id: "confirm", type: "button", label: "Confirm", primary: true, disabled: loading, loading, onClick: () => { void (clickedButton === "approve" ? transferTEI(school, selectedTei, handleCloseRequest) : rejectTEI(selectedTei?.transferInstance, handleCloseRequest)) } }
  ];
